/**
 * 
 * @TODO complete rewrite of jailed
 * 
 * 
 * 
...
(function (root, factory) {
    ...
    var basicConnectionNode = function() {
        var childProcess = undefined; //require('child_process');
...
m         * 
     ...
     *   - forceLoad : If forceLoad is set to true then initWeb.load will be 
     *        run on initialize since it means window.onload didn't run
     *   - setpath : If setpath is set then the path to jailed/lib becomes
     *        this path. 
     */
    var Plugin = function(url, _interface, _options) {
        this._path = url;
        this._initialInterface = _interface||{};
        this._initOpts = _options || {};
        
        if(_options['setpath']) __jailed__path__ = _options['setpath'];
        init();
        
        if(_options['forceload'])
            initWeb.onload();
        else
            window.onload = initWeb.onload();
